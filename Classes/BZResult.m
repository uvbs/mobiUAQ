...
@synthesize totalBytesFromNetwork;
...
- (void)startRequestForResource:(NSString*)identifier
{
    ...
    if (resource) {
        ...
        if (resource.startTime != 0) { 
            ...
            resource.blockedTime = [NSDate date];
            resource.dnsTime = [NSDate date];
        } else {
...
- (void)completeRequestForResource:(NSObject*)identifier
{
	...
	if (resource) {
        ...
        if (resource.endTime != 0) { 
            ...
            // JK add to calc download bytes from network
            self.totalBytesFromNetwork = self.totalBytesFromNetwork + resource.responseContentSizeFromNetwork;
        }
	}
	else {
