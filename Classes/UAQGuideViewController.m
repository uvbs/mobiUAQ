...
@interface UAQGuideViewController ()

@end

@implementation UAQGuideViewController
...
- (void)showGuide
{
    ...
    {
        [UAQGuideViewController sharedGuide].view.frame = [self offscreenFrame];
        [[self mainWindow] addSubview:[UAQGuideViewController sharedGuide].view];
        ...
        [UAQGuideViewController sharedGuide].view.frame = [self onscreenFrame];
...
- (void)hideGuide
{
    if(!_animating && self.view.superview != nil)
    {
        ...
        [UIView setAnimationDuration:0.4];
        [UIView setAnimationDelegate:self];
        [UIView setAnimationDidStopSelector:@selector(guideHide)];
        [UAQGuideViewController sharedGuide].view.frame = [self onscreenFrame];
        [UIView commitAnimations];
    }
}
...
{
    _animating = NO;
    [[[UAQGuideViewController sharedGuide] view] removeFromSuperview];
}
...
+ (void)show
{
    [[UAQGuideViewController sharedGuide].pageScroll setContentOffset:CGPointMake(0.f,0.f)];
    [[UAQGuideViewController sharedGuide] showGuide];
}

+ (void)hide
{
    [[UAQGuideViewController sharedGuide] hideGuide];
}
...
+ (UAQGuideViewController *)sharedGuide
{
    @synchronized(self)
    {
        static UAQGuideViewController *sharedGuide = nil;
        if(sharedGuide == nil)
        {
...
- (void)viewDidLoad
{
    ...
    for (int i=0; i<imageNameArray.count; i++) {
        ...
        if( i == imageNameArray.count - 1) {
            ...
            UIButton *enterButton = [[UIButton alloc] initWithFrame:CGRectMake(0.f, 0.f, 175, 35.f)];
