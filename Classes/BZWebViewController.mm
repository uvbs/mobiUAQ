...
//		--currentSubRun;
	...
	if (!preCache) {
        ...
        NSLog(@"JK sotre image in %@",sessionImageFolder);
	}
...
{	
	...
	NSLog(@"JK job.runs %i , fvOnly %i, timeOut %d",job.runs,job.fvOnly,didTimeout);
...
- (void)captureScreen:(NSString*)identifier important:(BOOL)important
{
	...
	CGImageRef screen = Nil;//= UIGetScreenImage();
    if (screen) {
